...
class Navbar extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      isMobile: window.innerWidth <= 768, // брейкпоинт
      menuOpen: false,
    };
            ...
            {menuOpen && (
              <div className=" resmenu w-100 ">
                ...
                <Link className="text-white bg-transparent themes" onClick={toggleTheme}>
                  {theme ? <h6>Темная тема <FaRegMoon /></h6> : <h6>Светлая тема <GoSun /></h6>}
                </Link>
                <Link to="/cart" className="ml-auto">
                  <ButtonContainer>
                    <i className="fas fa-cart-plus">Корзина</i>
                  </ButtonContainer>
                </Link>
              </div>
            )}
...
const MobileNavWrapper = styled(NavWrapper)`
  /* Телефонные стили тоже надо чет сделать */
`;

const DesktopNavWrapper = styled(NavWrapper)`
  /* пк стили, надо будет как-то сделать приятнее меню */
`;
